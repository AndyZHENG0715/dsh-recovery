...
/** Environment for a throwaway probe child: temp home, no browser-open
 * side channels, and none of the caller's live-session handles. */
function probeEnv(tmpHome) {
  const env = { ...process.env, DSH_HOME: tmpHome, NO_COLOR: '1' }
  for (const key of ['DSH_WEB_URL', 'DSH_WEB_MODE', 'DSH_SESSION_ID', 'DSH_SESSION_JSONL', 'DSH_SHELL']) delete env[key]
  return env
}
...
function runLiveProbe(tmpHome, profile, install, timeoutMs) {
  return new Promise((resolvePromise) => {
    ...
    probe.listen(0, '127.0.0.1', () => {
      ...
      probe.close(() => {
        // --no-open: the probe must never hand the temp URL to the user's
        // default browser (it dies seconds later). Port is always explicit, so
        // no default-port (3080) conflict or hint can appear either.
        const child = spawn(process.execPath, [dshBinOf(install), '--profile', profile, '--port', String(port), '--no-open'], {
          env: probeEnv(tmpHome),
