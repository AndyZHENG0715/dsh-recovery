...
export function safemodeEnter(home, opts = {}) {
  ...
  const snap = snapshot(home, { profile, reason: 'safemode-enter', install: opts.install, internal: false })
  const repaired = writeSafemodeProfile(home)
...
export function safemodeExit(home, opts = {}) {
  let repaired = []
  if (opts.reset === true) repaired = writeSafemodeProfile(home)
  const state = readState(home)
  state.mode = 'normal'
...
function writeSafemodeProfile(home) {
