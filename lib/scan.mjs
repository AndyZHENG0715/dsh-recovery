...
import { listQuarantined } from './patch-edit.mjs'
...
export function scanHome(home, opts = {}) {
  ...
  // ── P1 boot marker + quarantined rows ───────────────────────────────────
  const markerPath = join(home, 'recovery', 'boot-state.json')
  if (existsSync(markerPath)) {
    try {
      const marker = JSON.parse(readFileSync(markerPath, 'utf8'))
      add('info', 'boot-marker-present', 'boot marker present since ' + (marker.startedAt ?? '?') + ' (pid ' + (marker.pid ?? '?') + ') — an unclean shutdown may have left it', { path: markerPath })
    } catch {
      add('warning', 'boot-marker-corrupt', 'recovery/boot-state.json is not valid JSON', { path: markerPath })
    }
  }
  for (const quarantined of listQuarantined(profilePatchPath(home, profile))) {
    add('info', 'quarantined-row', 'row ' + JSON.stringify(quarantined.id) + ' is quarantined by dsh-recovery (patch line ' + quarantined.line + ') — undo with: dsh-recovery unquarantine --id ' + quarantined.id)
  }
